define(["map/clientlayer", "map/labelslayer",
  ...
  function (ClientLayer, LabelsLayer, d3, L, moment, LocationMarker, rbush) {
    ...
    var LocationPickerButton = L.Control.extend({
      options: {
        position: "bottomright"
      },
      ...
      onAdd: function () {
        var button = L.DomUtil.create("button", "location-picker")
        button.textContent = ""
    ...
    return function (config, linkScale, sidebar, router, buttons) {
      ...
      var locationPickerButton = new LocationPickerButton(function (d) {
        if (d)
          enableCoordinates()
        else
          disableCoordinates()
      ...
      function enableCoordinates() {
        map.getContainer().classList.add("pick-location")
        map.on("click", showCoordinates)
        locationPickerButton.set(true)
      }

      function disableCoordinates() {
        map.getContainer().classList.remove("pick-location")
        map.off("click", showCoordinates)
        locationPickerButton.set(false)
      }
      ...
      addButton(locationPickerButton)
